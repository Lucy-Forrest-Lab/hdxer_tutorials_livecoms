...
%DIF DEL hdxer_tutorials_maintext_v0.1.tex   Wed Dec  1 09:42:47 2021
%DIF ADD hdxer_tutorials_maintext_v0.2.tex   Fri Dec  3 17:14:32 2021
...
%DIF -------
\newcommand{\versionnumber}{0.2}  % you should update the minor version number in preprints and major version number of submissions. %DIF > 
\newcommand{\githubrepository}{\url{https://github.com/Lucy-Forrest-Lab/hdxer_tutorials_livecoms}} %DIF > 
%DIF -------
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%DIF 33c33
%DIF < \title{Combining hydrogen-deuterium exchange experiments with molecular simulations: Tutorials and applications of the HDXer ensemble reweighting software [Article v\versionnumber]}
%DIF -------
\title{Interpreting hydrogen-deuterium exchange experiments with molecular simulations: Tutorials and applications of the HDXer ensemble reweighting software [Article v\versionnumber]} %DIF > 
%DIF -------
...
%DIF 45c45
%DIF < \affil[2]{Theoretical Molecular Biophysics Section, National Heart, Lung, and Blood Institute, National Institutes of Health, Bethesda, MD, USA}
%DIF -------
\affil[2]{Theoretical Molecular Biophysics Laboratory, National Heart, Lung, and Blood Institute, National Institutes of Health, Bethesda, MD, USA} %DIF > 
%DIF -------
...
\begin{abstract}
Hydrogen-deuterium exchange (HDX) \DIFdelbegin \DIFdel{in proteins is a subtle yet detailed reporter }\DIFdelend \DIFaddbegin \DIFadd{is a comprehensive yet detailed probe }\DIFaddend of protein structure and dynamics \DIFdelbegin \DIFdel{, }\DIFdelend and, coupled to mass spectrometry, has become a powerful \DIFdelbegin \DIFdel{structural biology tool for probing }\DIFdelend \DIFaddbegin \DIFadd{tool for investigating }\DIFaddend an increasingly large array of \DIFdelbegin \DIFdel{protein }\DIFdelend systems.
Computer simulations are often used to help rationalize experimental observations of exchange, but interpretations have frequently been limited to simple, subjective \DIFdelbegin \DIFdel{, }\DIFdelend correlations between microscopic dynamical fluctuations and the observed macroscopic exchange behavior.
With this in mind, \DIFaddbegin \DIFadd{we previously developed }\DIFaddend the HDX ensemble reweighting approach and associated software, HDXer, \DIFdelbegin \DIFdel{has been designed }\DIFdelend to aid the objective interpretation of HDX data using molecular simulations.
HDXer has two main functions; first, to \DIFdelbegin \DIFdel{predict }\DIFdelend \DIFaddbegin \DIFadd{compute }\DIFaddend H-D exchange rates \DIFdelbegin \DIFdel{from }\DIFdelend \DIFaddbegin \DIFadd{that describe each structure in }\DIFaddend a candidate ensemble of protein structures, for example from molecular simulations, and second, to objectively reweight the conformational populations present in a candidate ensemble to conform to experimental exchange data.
In this article, we first describe the HDXer approach, theory, and implementation. 
We then guide users through a suite of tutorials \DIFdelbegin \DIFdel{, demonstrating practical applications of experimental data preparation, predicting }\DIFdelend \DIFaddbegin \DIFadd{that demonstrate the practical aspects of preparing experimental data, computing }\DIFaddend HDX levels from molecular simulations, and performing ensemble reweighting analyses.
Finally \DIFdelbegin \DIFdel{, }\DIFdelend we provide a practical discussion of the capabilities and limitations of the HDXer methods \DIFdelbegin \DIFdel{, }\DIFdelend including recommendations for a user's own analyses.
Overall, this article is intended to provide an up-to-date, pedagogical counterpart to the software, which is freely available at \DIFdelbegin %DIFDELCMD < \url{https://github.com/TMB-CSB/HDXer}%%%
\DIFdelend \DIFaddbegin \url{https://github.com/Lucy-Forrest-Lab/HDXer}\DIFaddend .
\end{abstract}
...
A pioneering descriptive framework \DIFdelbegin \DIFdel{, developed }\DIFdelend \DIFaddbegin \DIFadd{developed in the 1950s }\DIFaddend by Linderstrøm-Lang and co-workers \DIFdelbegin \DIFdel{, }\DIFdelend first linked the hydrogen exchange rates of backbone amide functional groups directly to protein structure  \DIFdelbegin \DIFdel{in the 1950s }\DIFdelend \cite{Hvidt1954, Englander1997}. 
This concept provided the foundation for the subsequent development of hydrogen-deuterium exchange (HDX) measurements as a sensitive, label-free probe of protein conformation and dynamics in solution \DIFdelbegin \DIFdel{, }\DIFdelend and due to these advances HDX is now a valuable addition to the modern structural biology toolbox. 
...
In particular, experiments that couple HDX to mass spectrometry measurements of the deuterium uptake (HDX-MS) \DIFdelbegin \DIFdel{have become the }\textit{\DIFdel{de facto}} %DIFAUXCMD
\DIFdel{standard for applications of HDX to }\DIFdelend \DIFaddbegin \DIFadd{are now widely used for analyzing }\DIFaddend the dynamics of proteins in solution
\DIFdelbegin \DIFdel{, allowing determination of exchange data for large protein systems or complexesand with low sample amounts \mbox{%DIFAUXCMD
\cite{Oganesyan2018, Engen2020}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{HDX-MS offers particular advantages over methods such as NMR-based exchange measurements for large proteins, complexes, and for small sample amounts \mbox{%DIFAUXCMD
\cite{Oganesyan2018, Engen2020, James2021}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
After completing the tutorials, which are available alongside the HDXer software itself in the GitHub repository (\DIFdelbegin %DIFDELCMD < \url{https://github.com/TMB-CSB/HDXer}%%%
...
\item compute \DIFdelbegin \DIFdel{predicted }\DIFdelend per-residue Protection Factor values and \DIFaddbegin \DIFadd{corresponding }\DIFaddend deuterated fractions for peptide segments \DIFdelbegin \DIFdel{from }\DIFdelend \DIFaddbegin \DIFadd{for each structure in }\DIFaddend an ensemble of candidate \DIFdelbegin \DIFdel{structures }\DIFdelend \DIFaddbegin \DIFadd{conformations }\DIFaddend (obtained, \DIFdelbegin \DIFdel{for example}\DIFdelend \DIFaddbegin \DIFadd{e.g.}\DIFaddend , from molecular dynamics (MD) simulations); 
\item reweight a candidate structural ensemble so that its \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{computed }\DIFaddend HDX levels fit those in a target set of experimental HDX-MS data; 
...
In terms of background knowledge, we \DIFdelbegin \DIFdel{begin here with }\DIFdelend \DIFaddbegin \DIFadd{provide }\DIFaddend a general introduction to the theoretical concepts of both HDX and ensemble reweighting \DIFdelbegin \DIFdel{, while suggesting }\DIFdelend \DIFaddbegin \DIFadd{in Section \ref{interpretingexptdata_sect}, which also points to }\DIFaddend sources of more detailed information for the interested user.

\DIFaddbegin \subsection{\DIFadd{Tutorial prerequisites}}
\subsubsection{\DIFadd{Background knowledge}}
\DIFadd{Our explanations of the HDXer method and tutorials assume that users are familiar with common ideas and terminology of physical chemistry, thermodynamics, and statistical ensembles of molecules.
...
Nevertheless, users are expected to have a good understanding of the two input data sources for HDXer analyses, namely structural ensembles generated }\textit{\DIFadd{via}} \DIFadd{molecular modeling and simulation, and the experimental measurement of protein HDX-MS data. 
}

\DIFadd{In practical terms, users should be familiar with the process of preparing and performing simulations, or generating models, with their chosen software.   
...
\DIFadd{In terms of computational background, to benefit from the tutorials users will need a basic level of familiarity with Unix environments and the command line interface (e.g., making and moving files and directories).
...
\DIFadd{Finally, users should be familiar with their target experimental HDX data, including the conditions under which it was obtained, the level of deuterium back-exchange (for HDX-MS data), and the proposed kinetic mechanism of exchange for each amide or peptide.
...
\subsubsection{\DIFadd{Software}}\label{Software_sect}
\DIFadd{The HDXer tutorials are written in Python 3.7 and are available as Jupyter notebooks downloadable from the HDXer GitHub repository at }\url{https://github.com/Lucy-Forrest-Lab/HDXer}\DIFadd{.
Users should use the git version control software to clone and update the HDXer package. Installation instructions are documented in the }\texttt{\DIFadd{README}} \DIFadd{file within the repository.
We recommend creating a separate Python virtual environment for HDXer analyses, with the following dependencies: MDTraj ($\geqslant$1.9.3) \mbox{%DIFAUXCMD
\cite{McGibbon2015MDTraj}}\hspace{0pt}%DIFAUXCMD
, NumPy ($\geqslant$1.18.1) \mbox{%DIFAUXCMD
\cite{Harris2020}}\hspace{0pt}%DIFAUXCMD
, Matplotlib ($\geqslant$3.2.0) \mbox{%DIFAUXCMD
\cite{Hunter:2007}}\hspace{0pt}%DIFAUXCMD
, Pytest ($\geqslant$5.3.5) \mbox{%DIFAUXCMD
\cite{pytest5.3}}\hspace{0pt}%DIFAUXCMD
, and Jupyterlab ($\geqslant$3.0) \mbox{%DIFAUXCMD
\cite{project_jupyter}}\hspace{0pt}%DIFAUXCMD
.
To recreate the testing environment for HDXer, a Conda environment file named }\texttt{\DIFadd{HDXER\_ENV.yml}} \DIFadd{is available in the HDXer GitHub repository.
...
\subsubsection{\DIFadd{Hardware}}
\DIFadd{The HDXer tutorials have modest hardware requirements and are designed to work with any relatively modern consumer desktop or laptop. 
...
\begin{figure}
    \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.98\linewidth]{Fig1_HDXer_schematic_v4.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.98\linewidth]{Fig1_HDXer_schematic_v5.pdf}
...
Atomic-resolution MD simulations initiated with a high-resolution, experimentally-determined structure \DIFdelbegin \DIFdel{, }\DIFdelend constitute a well-established approach to explore the conformational space of biomolecules in solution \cite{Huggins2019}. 
Such simulations \DIFdelbegin \DIFdel{, by definition , }\DIFdelend \DIFaddbegin \DIFadd{by definition }\DIFaddend sample a Boltzmann distribution of states, making them a straightforward and rigorous tool for generating a candidate ensemble of structures for HDXer analyses \DIFdelbegin \DIFdel{, }\DIFdelend and as such have been used in the HDXer tutorials described here.
...
In some cases, therefore, classical (unbiased) simulations may not sufficiently sample the relevant conformational space \DIFdelbegin \DIFdel{, }\DIFdelend or may become kinetically trapped in regions of conformational space that are not representative of the dynamics observed experimentally.
...
\DIFdelbegin \DIFdel{A straightforward approach, for example, }\DIFdelend \DIFaddbegin \DIFadd{For example, a straightforward approach }\DIFaddend would be to combine ensembles from multiple simulations, each initiated with a differing structure.
Such a strategy would enable the inclusion of any relevant alternative (e.g. active vs. inactive \DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{or }\DIFaddend apo vs. holo) conformational states that may not be accessed during a simulation initiated with only one of those states.
...
However, the \DIFdelbegin \DIFdel{further }\DIFdelend \DIFaddbegin \DIFadd{more dissimilar }\DIFaddend the initial set of candidate structures is \DIFdelbegin \DIFdel{from }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend an equilibrium Boltzmann distribution of states, the greater the level of bias that is likely required to reweight the ensemble to conform to the experimental data, because many poorly-conforming structures will need to be downweighted.
...
\DIFdelbegin \DIFdel{Since the environment and dynamics of }\DIFdelend \DIFaddbegin \DIFadd{Within the Linderstrøm-Lang model for hydrogen exchange, }\DIFaddend each backbone amide \DIFdelbegin \DIFdel{in a protein chain are unique, the underlying structural and chemical mechanisms of exchange within a protein may vary \mbox{%DIFAUXCMD
\cite{Englander1983, Skinner2012mechanisms, Skinner2012models}}\hspace{0pt}%DIFAUXCMD
.
As such, there are no rigorous, generalized structural determinants of exchange}\DIFdelend \DIFaddbegin \DIFadd{exists in either an 'open', accessible and exchange-competent state (O), or a 'closed', protected and exchange-non-competent state (C) \mbox{%DIFAUXCMD
\cite{Hvidt1966, Englander1997, Jensen2016}}\hspace{0pt}%DIFAUXCMD
.
...
An examination of the relationship between these rate constants (Equation \ref{LL_exch_model_eqn}) reveals that the experimentally-observed overall rate of exchange, $k_{\textnormal{HX}}$ can be considered to occupy one of two alternative kinetic regimes, known as EX1 and EX2 (Equation \ref{EX1_EX2_eqn}) \mbox{%DIFAUXCMD
\cite{Jensen2016, Ferraro2004, James2021}}\hspace{0pt}%DIFAUXCMD
.
}

\begin{equation}\DIFadd{\label{LL_exch_model_eqn}
}\ce{(N-H)_C <=>[$k$_{op}][$k$_{cl}] (N-H)_O <=>[$k$_{int}][] (N-D)_O <=>[$k$_{cl}][$k$_{op}] (N-D)_C}
\DIFadd{}\end{equation}
\begin{equation}\DIFadd{\label{EX1_EX2_eqn}
\begin{gathered}
...
\DIFadd{Amides can require vastly different structural transitions to reach an exchange-competent O state \mbox{%DIFAUXCMD
\cite{Hvidt1966, Englander1983, Skinner2012mechanisms, Skinner2012models}}\hspace{0pt}%DIFAUXCMD
. 
...
Amides in these structures will therefore undergo exchange with EX1 kinetics, with an overall rate directly proportional to that of the unfolding conformational change, $k_\textnormal{op}$}\DIFaddend .
\DIFdelbegin \DIFdel{However, a number of predictive models have been developed to correlate }\DIFdelend \DIFaddbegin 

\DIFadd{Amides that exhibit EX2 exchange access their exchange-competent states }\textit{\DIFadd{via}} \DIFadd{smaller conformational fluctuations, in which the rate of refolding to the C state is far faster than the chemical exchange reaction.
...
\begin{equation}\DIFadd{\label{PF_deltaG_eqn}
}{\DIFadd{\Delta}}\DIFadd{G_{\textnormal{op}} = -RT\ln{\frac{k_{\textnormal{op}}}{k_{\textnormal{cl}}}} = RT\ln{P_i}
}\end{equation}

\DIFadd{Predictive models to estimate protection factors provide an opportunity to interpret EX2 exchange in a structural context, and typically fall into two families. 
The first type of approach requires extensive sampling of the amide C and O states, from which the C/O equilibrium constant $P_i$ is calculated directly by classifying each conformation as 'open' or 'closed' and then counting the relative populations (i.e. probabilities) of the two states \mbox{%DIFAUXCMD
\cite{Persson2015, Liu2012}}\hspace{0pt}%DIFAUXCMD
.
A common alternative technique is to use an empirical scoring function that correlates the amide $\Delta G_\textnormal{op}$ with }\DIFaddend protein structural and dynamical \DIFdelbegin \DIFdel{metrics with protection factors \mbox{%DIFAUXCMD
\cite{VendruscoloPaci2003, BestVendruscolo2006, Kieseritzky2006, Ma2011, Liu2012, Petruk2013, Park2015, Persson2015, Markwick2019}}\hspace{0pt}%DIFAUXCMD
.
The protection factors estimated by these models may then be used to calculate exchange rates, }\DIFdelend \DIFaddbegin \DIFadd{features observed in an ensemble of structures  \mbox{%DIFAUXCMD
\cite{VendruscoloPaci2003, BestVendruscolo2006, Kieseritzky2006, Ma2011, Petruk2013, Park2015, Markwick2019}}\hspace{0pt}%DIFAUXCMD
.
...
Consequently, the experimental data used to interpret }\textit{\DIFadd{via}} \DIFadd{predictive models should be carefully classified according to EX1 }\DIFaddend and \DIFdelbegin \DIFdel{ultimately deuterium uptake, for peptides observed to follow the }\DIFdelend EX2 \DIFdelbegin \DIFdel{exchange kinetic regime }\DIFdelend \DIFaddbegin \DIFadd{behavior so that the slower exchanging amides may be excluded.
Fortunately, a simple analysis of the peptide mass envelopes obtained }\DIFaddend in HDX-MS experiments \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{often allows classification of individual peptides into EX1, EX2, or mixed EX1/EX2 exchangers for this purpose \mbox{%DIFAUXCMD
\cite{Jensen2016, Ferraro2004, James2021}}\hspace{0pt}%DIFAUXCMD
.
}
...
At its core, the HDXer process applies an energy bias to the underlying potential energy of each \DIFdelbegin \DIFdel{frame }\DIFdelend \DIFaddbegin \DIFadd{structure }\DIFaddend in the initial candidate ensemble, $U(X)$, to create a new, corrected, potential, $U_{\textnormal{corr}}(X)$:
...
\end{equation}
The values of the scaling parameters $\lambda_i$ for each residue, along with the number of contacts and H bonds $N_{\textnormal{C},i}$ and $N_{\textnormal{H},i}$ formed by that residue, determine the bias applied to each \DIFdelbegin \DIFdel{frame of the ensemble , }\DIFdelend \DIFaddbegin \DIFadd{structure in the ensemble }\DIFaddend and ultimately the new, corrected, contribution of each \DIFdelbegin \DIFdel{frame }\DIFdelend \DIFaddbegin \DIFadd{structure }\DIFaddend to the overall population.
...
To do so, HDXer iteratively minimizes a \DIFdelbegin \DIFdel{likelihood }\DIFdelend \DIFaddbegin \DIFadd{distance }\DIFaddend function that balances the bias applied, quantified across the whole ensemble as an apparent work, $W_\textnormal{app}$, with an error distribution, $\ln{\rho_\textnormal{err}}$:
\DIFaddbegin 

\DIFaddend \begin{equation}\label{KL_eqn}
\DIFdelbegin \DIFdel{L = \frac{W_\textnormal{app}}{k_\textnormal{B}T} - \ln{\rho_\textnormal{err}},
...
\DIFaddend where $T$ is the temperature\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend $k_\textnormal{B}$ is the Boltzmann constant\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, $D_{j,t}^\textnormal{sim}$ and $D_{j,t}^\textnormal{exp}$ are the simulated and experimental deuteration for peptide $j$ at time $t$, and $\eta$ is an estimate of the uncertainty.
Here $\eta$ is set to 1, such that $\gamma$ describes the uncertainty for all target data points.
}

\DIFaddend The error distribution is applied to the difference between the calculated and target deuteration levels, meaning that in order to minimize the \DIFdelbegin \DIFdel{likelihood function in equation }\DIFdelend \DIFaddbegin \DIFadd{distance function in Equation }\DIFaddend \ref{KL_eqn}, the reweighting process must decrease the difference between calculated and target HDX. 
...
The final difference between calculated and target deuteration levels is balanced with the bias applied to the candidate ensemble, $W_\textnormal{app}$, which must be simultaneously minimized in order to minimize the \DIFdelbegin \DIFdel{likelihood function in equation }\DIFdelend \DIFaddbegin \DIFadd{distance function in Equation }\DIFaddend \ref{KL_eqn}. 
...
The width of each distribution, and hence the uncertainty in the data, can be controlled with \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend single user-defined parameter \DIFdelbegin \DIFdel{, }\DIFdelend $\gamma$ \DIFaddbegin \DIFadd{in Equation \ref{KL_eqn}}\DIFaddend .

Although brief, we expect that the above description will help users to understand the rationale for the key inputs and outputs of HDXer analyses, \DIFaddbegin \DIFadd{which are }\DIFaddend summarized below.

The inputs for a maximum entropy reweighting tool were first introduced conceptually in \DIFdelbegin \DIFdel{section }\DIFdelend \DIFaddbegin \DIFadd{Section }\DIFaddend \ref{interpretingexptdata_sect}. More specifically, HDXer analyses require:
\begin{itemize}
    \item an atomistic structural ensemble of a protein of interest, in the relevant environment \DIFdelbegin \DIFdel{, }\DIFdelend and with relevant ligands/co-solutes if desired;
...
\begin{itemize}
    ...
    \item the apparent work, $W_{app}$ \DIFdelbegin \DIFdel{, }\DIFdelend that was applied to the reweighted ensemble, as a measure of the total bias applied and the degree to which the distribution of frames in the candidate ensemble was shifted by the reweighting process.
\end{itemize}
...
We detail the \DIFdelbegin \DIFdel{remaining, more practical, prerequisites for the tutorials and HDXer analyses }\DIFdelend \DIFaddbegin \DIFadd{workflow of a typical HDXer analysis }\DIFaddend in the following section\DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin \DIFadd{, and the remaining practical requirements for the tutorials and HDXer analyses in Section \ref{collating_inputs}.
}\DIFaddend 
...
\DIFdel{In addition to a solid foundation in the theory underpinning ensemble reweighting and computational HDX predictions, users should have a good understanding of the two data sources in their HDXer analyses, namely structural ensembles generated }\textit{\DIFdel{via}} %DIFAUXCMD
...
%DIFDELCMD < %%%
\subsubsection{\DIFdel{Software}}%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < \label{Software_sect}
%DIFDELCMD < %%%
\DIFdel{The HDXer tutorials are written in Python 3.7, and are available as Jupyter notebooks, downloadable from the HDXer GitHub repository at }%DIFDELCMD < \url{https://github.com/TMB-CSB/HDXer}%%%
\DIFdel{.
Users should use the git version control software to clone and update the HDXer package. Installation instructions are documented in the }\texttt{\DIFdel{README}} %DIFAUXCMD
\DIFdel{file within the repository.
We recommend creating a separate Python virtual environment for HDXer analyses, which has the following dependencies: MDTraj ($\geqslant$1.9.3) \mbox{%DIFAUXCMD
\cite{McGibbon2015MDTraj}}\hspace{0pt}%DIFAUXCMD
, NumPy ($\geqslant$1.18.1) \mbox{%DIFAUXCMD
\cite{Harris2020}}\hspace{0pt}%DIFAUXCMD
, Matplotlib ($\geqslant$3.2.0) \mbox{%DIFAUXCMD
...
\cite{project_jupyter}}\hspace{0pt}%DIFAUXCMD
.
To recreate the testing environment for HDXer, a Conda environment file, }\texttt{\DIFdel{HDXER\_ENV.yml}}%DIFAUXCMD
...
%DIFDELCMD <     \includegraphics[width=0.98\linewidth]{Fig2_HDXer_workflow_vertical.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \begin{figure}[t]
    \includegraphics[width=0.98\linewidth]{Fig2_HDXer_workflow_vertical_v2.pdf}
    \DIFaddendFL \caption{Workflow of a typical HDXer analysis. Input structural and experimentally-measured HDX-MS (orange; the dashed line denotes an optional input) can be processed through multiple analysis steps (blue), to obtain \DIFdelbeginFL \DIFdelFL{predicted }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{calculated }\DIFaddendFL HDX-MS data and/or a reweighted structural ensemble as outputs (green). The corresponding software packages or python functions are listed alongside each method. The tutorials cover all aspects of HDXer usage, from preparing input data to interpreting the outputs.}
    \label{fig:workflowfig}
\end{figure}

\subsection{Organization of the tutorials}
The initial stage for any HDXer analysis is the collation and preparation of input data sources, such as the candidate structural ensemble and target (experimental) HDX-MS data, as specified in \DIFdelbegin \DIFdel{section }\DIFdelend \DIFaddbegin \DIFadd{Section }\DIFaddend \ref{maxent_theory_sect}. 
We describe how to collate such input data in \DIFdelbegin \DIFdel{section }\DIFdelend \DIFaddbegin \DIFadd{Section }\DIFaddend \ref{collating_inputs}, while tutorial 1 (\DIFdelbegin \DIFdel{section }\DIFdelend \DIFaddbegin \DIFadd{Section }\DIFaddend \ref{tutorial_1}) provides a practical example of the process, with particular focus on dealing with experimental data from diverse sources.

Having prepared suitable input data, the next stage of an HDXer analysis involves the \DIFdelbegin \DIFdel{prediction }\DIFdelend \DIFaddbegin \DIFadd{calculation }\DIFaddend of protection factors from the candidate structural ensemble, the subsequent calculation of \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{corresponding }\DIFaddend deuteration levels for peptide segments, and comparison with experimental HDX-MS data if desired.
Tutorial 2 (\DIFdelbegin \DIFdel{section }\DIFdelend \DIFaddbegin \DIFadd{Section }\DIFaddend \ref{tutorial_2}) demonstrates an application of the protection factor and HDX-MS data \DIFdelbegin \DIFdel{prediction }\DIFdelend \DIFaddbegin \DIFadd{calculation }\DIFaddend functionalities.

The final possible HDXer analysis is to reweight the candidate ensemble to fit target (experimental) HDX-MS data.
Tutorial 3 (\DIFdelbegin \DIFdel{section }\DIFdelend \DIFaddbegin \DIFadd{Section }\DIFaddend \ref{tutorial_3}) describes an example application based on the HDX-MS \DIFdelbegin \DIFdel{predictions }\DIFdelend \DIFaddbegin \DIFadd{calculations }\DIFaddend completed in tutorial 2, and illustrates how to interpret the outputs of reweighting, namely the final structural ensemble, the reweighted HDX-MS data, and the metrics of $W_\textnormal{app}$ and RMSE to the target data. 
...
However, \DIFdelbegin \DIFdel{any datapoints that }\DIFdelend \DIFaddbegin \DIFadd{the information content of all experimental datapoints is unlikely to be equal.
...
Overall then, datapoints that }\DIFaddend do not exhibit significant differences in mass during the \DIFdelbegin \DIFdel{experiment can }\DIFdelend \DIFaddbegin \DIFadd{experimental timecourse could }\DIFaddend effectively decrease the signal-to-noise over the entire ensemble \DIFdelbegin \DIFdel{, given the realities of real-life, noisy, experimental data}\DIFdelend \DIFaddbegin \DIFadd{and result in ambiguous structural interpretations}\DIFaddend .
Therefore, in practice, \DIFdelbegin \DIFdel{we recommend that only datapoints for which significant deuteration are reported be included in the target dataset (see }\DIFdelend \DIFaddbegin \DIFadd{minimizing the number of datapoints without significant significant changes in deuteration over the experimental time course is likely to improve the structural fidelity of reweighting analyses. 
See also }\DIFaddend e.g. \cite{Hageman2019, Weis2021} for useful methods to determine significance in HDX-MS data\DIFdelbegin \DIFdel{).
This advice applies also to data being }\DIFdelend \DIFaddbegin \DIFadd{.
}

\DIFadd{For data }\DIFaddend used to compare ensembles obtained for different states, i.e., when reweighting ensembles from experiments carried out under different protein conditions, e.g. apo and holo states\DIFaddbegin \DIFadd{, similar considerations apply}\DIFaddend . Specifically, \DIFdelbegin \DIFdel{we recommend including only }\DIFdelend datapoints that show significant mass differences between the two conditions (i.e.\DIFaddbegin \DIFadd{, }\DIFaddend significant $\Delta$deuteration) \DIFdelbegin \DIFdel{.
With this approach, }\DIFdelend \DIFaddbegin \DIFadd{are most informative for reweighting.
...
Thus, it can be advantageous to minimize the number of peptides with non-significant $\Delta$HDX, in which case }\DIFaddend only the datapoints that contribute most to the differences between states drive the outcome of the reweighting.

\DIFaddbegin \DIFadd{Nevertheless, filtering of the target experimental data may be undesirable if it means key structural regions of interest are removed from the target dataset.
...
Careful optimization of experimental timepoints, temperature, pD, peptide coverage, and redundancy is therefore always recommended as part of the data curation process for HDXer analyses, and for HDX-MS experiments in general \mbox{%DIFAUXCMD
\cite{Masson2019}}\hspace{0pt}%DIFAUXCMD
.
Further suggestions in this regard are provided in Section \ref{expt_design_sect}.
}

\DIFaddend \subsection{Curating a structural ensemble}
As mentioned in \DIFdelbegin \DIFdel{section }\DIFdelend \DIFaddbegin \DIFadd{Section }\DIFaddend \ref{initialensemble_sect}, there are numerous methodologies that can be used to prepare a suitable candidate protein structural ensemble for reweighting.
However, we expect that most HDXer applications will make use of protein structures generated by MD simulations.
HDXer relies upon the MDTraj package to read and analyze biomolecular structures and trajectories, \DIFdelbegin \DIFdel{conferring }\DIFdelend \DIFaddbegin \DIFadd{which confers }\DIFaddend the ability to read coordinate and topology file formats used by most major MD software packages.
...
Motions across $\mu$s to ms timescales\DIFaddbegin \DIFadd{, for example the refolding of local structural features in a Heme binding protein \mbox{%DIFAUXCMD
\cite{Kihn2021}}\hspace{0pt}%DIFAUXCMD
, }\DIFaddend are likely to require enhanced sampling \DIFaddbegin \DIFadd{approaches }\DIFaddend \cite{Allison2020} (although note that motions only seen on even longer timescales may be associated with global unfolding and EX1-type kinetics, which is as yet unsuitable for HDXer analysis). 
...
Specifically, bias-exchange metadynamics simulations had been used to \DIFdelbegin \DIFdel{sample }\DIFdelend \DIFaddbegin \DIFadd{explore }\DIFaddend the complete free-energy landscape of the TeaA conformational changes \cite{Marinelli2011}, but only the unbiased replica of the simulations, which represents the equilibrium dynamics, was used as the prototypical candidate structural ensemble during reweighting \cite{Bradshaw2020}.
An alternative method, for simulation techniques that do not include an unbiased ensemble, would be for users to ensure \DIFaddbegin \DIFadd{that }\DIFaddend the initial weights of their frames reproduce a Boltzmann distribution \cite{Torrie1977, Marinelli2021}.
Instructions for assigning initial weights can be found in the HDXer 'docstring' help text\DIFaddbegin \DIFadd{, accessible in an interactive Python interpreter session by running:  }\texttt{\DIFadd{help(HDXer.reweighting.MaxEnt.run)}}\DIFaddend .
...
HDXer is highly customizable \DIFdelbegin \DIFdel{, }\DIFdelend and almost all the options used during an analysis can be user-specified. 
...
\subsection{Parameters for \DIFdelbegin \DIFdel{predicting }\DIFdelend \DIFaddbegin \DIFadd{computing }\DIFaddend HDX-MS data from structures}\label{choosingbetaparams_sect}
The predictive model (\DIFdelbegin \DIFdel{eqn. }\DIFdelend \DIFaddbegin \DIFadd{Equation }\DIFaddend \ref{pf_model}) used by HDXer to \DIFdelbegin \DIFdel{predict }\DIFdelend \DIFaddbegin \DIFadd{compute }\DIFaddend protection factors from molecular structures includes two key scaling factors, $\beta_\textnormal{C}$ and $\beta_\textnormal{H}$.
These factors determine the relative contributions of H bonds and contacts to the structural protection attributed to the protein fold \DIFdelbegin \DIFdel{, }\DIFdelend and ultimately define the estimated $\Delta$G of the conformational motions resulting in exchange \cite{VendruscoloPaci2003, BestVendruscolo2006}.
The relative importance of these two structural factors to the energy may vary\DIFdelbegin \DIFdel{between, and within}\DIFdelend , \DIFaddbegin \DIFadd{both between and within }\DIFaddend proteins, and so the optimum values of the scaling factors in our empirical predictive model may also vary.
As \DIFdelbegin \DIFdel{we introduced in section }\DIFdelend \DIFaddbegin \DIFadd{introduced in Section }\DIFaddend \ref{predictivemodel_sect}, the default scaling factors used by HDXer ($\beta_\textnormal{C} = 0.35$, $\beta_\textnormal{H} = 2.0$) were originally parameterized using protection factor data for small, globular proteins \cite{BestVendruscolo2006}.
However, for HDX \DIFdelbegin \DIFdel{predictions }\DIFdelend \DIFaddbegin \DIFadd{calculations }\DIFaddend for larger proteins, or those in non-aqueous environments, the default values may not be suitable \DIFdelbegin \DIFdel{, }\DIFdelend and we therefore recommend that users explore the effects of changing $\beta_\textnormal{C}$ and $\beta_\textnormal{H}$ before embarking upon intensive reweighting analyses.
...
Detailed installation instructions can be found on the main page of the HDXer GitHub repository (\DIFdelbegin %DIFDELCMD < \url{https://github.com/TMB-CSB/HDXer}%%%
...
In this tutorial we will further convert the experimental protection factors into fractional deuteration values for each residue, as if the data had been recorded \DIFdelbegin \textit{\DIFdel{via}} %DIFAUXCMD
...
This is because HDXer assumes that any deuterium labeling of the first residue in each HDX-MS peptide segment is completely back-exchanged to hydrogen after the quenching of labeling and digestion of the intact protein, as the hydrolyzed amide \DIFdelbegin \DIFdel{becomes a new, }\DIFdelend \DIFaddbegin \DIFadd{is converted to an }\DIFaddend extremely labile, N-terminal amine \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Bai1993, Walters2012}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
The next stage involves \DIFdelbegin \DIFdel{predicting }\DIFdelend \DIFaddbegin \DIFadd{generating }\DIFaddend HDX data from \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a candidate }\DIFaddend structural ensemble that is to be reweighted.

% Section 5.3
\subsection{Tutorial 2: \DIFdelbegin \DIFdel{Predicting }\DIFdelend \DIFaddbegin \DIFadd{Producing a structural ensemble and associated }\DIFaddend HDX data}\label{tutorial_2}

\subsubsection{Introduction}
The goal of \DIFdelbegin \DIFdel{predicting deuterium exchange from structure is to allow direct comparison of predicted and }\DIFdelend \DIFaddbegin \DIFadd{HDXer is to facilitate the structural interpretation of }\DIFaddend experimental HDX data \DIFdelbegin \DIFdel{, and thereby interpretation of the observed HDX }\DIFdelend in molecular detail. 
\DIFdelbegin \DIFdel{In the }\DIFdelend \DIFaddbegin \DIFadd{To do so, we generate an ensemble of structures, calculate the corresponding HDX levels according to a predictive model, and manipulate the ensemble to highlight the configurations of that ensemble that are most representative of the data. 
The }\DIFaddend second Jupyter notebook, \texttt{02\_calc\_hdx.ipynb} \DIFdelbegin \DIFdel{, we provide details of }\DIFdelend \DIFaddbegin \DIFadd{provides an example ensemble of structures for BPTI, explains }\DIFaddend the predictive model used \DIFdelbegin \DIFdel{by HDXer }\DIFdelend to estimate protection factors and \DIFdelbegin \DIFdel{predict }\DIFdelend \DIFaddbegin \DIFadd{corresponding }\DIFaddend HDX-MS data \DIFdelbegin \DIFdel{, and guide users through an example }\DIFdelend \DIFaddbegin \DIFadd{for each snapshot, and guides users through such an }\DIFaddend HDX-MS \DIFdelbegin \DIFdel{prediction using an ensemble of structures of }\DIFdelend \DIFaddbegin \DIFadd{calculation for }\DIFaddend BPTI.
The \DIFdelbegin \DIFdel{computational predictions }\DIFdelend \DIFaddbegin \DIFadd{computed HDX values }\DIFaddend will then be compared with the experimental BPTI data that were prepared during Tutorial 1.

\subsubsection{Prerequisites} 
The key requirement for this tutorial is an ensemble of atomistic \DIFdelbegin \DIFdel{protein structures }\DIFdelend \DIFaddbegin \DIFadd{structures for BPTI}\DIFaddend , which will be used both for \DIFdelbegin \DIFdel{predicting }\DIFdelend \DIFaddbegin \DIFadd{computing }\DIFaddend HDX levels and for the later reweighting steps. 
For BPTI, we provide in-house MD simulation trajectories in a repository on Zenodo (\DIFdelbegin %DIFDELCMD < \url{doi:10.5281/zenodo.4640760}%%%
\DIFdelend \DIFaddbegin \url{https://dx.doi.org/10.5281/zenodo.4640760}\DIFaddend ). 
This repository contains five replica trajectories, each 500 ns long, from five separate simulations of BPTI in solution. 
To follow along with all the steps in this tutorial notebook, users should download the entire directory containing all five runs (i.e., \texttt{BPTI\_simulations}) \DIFdelbegin \DIFdel{, from Zenodo , }\DIFdelend \DIFaddbegin \DIFadd{from Zenodo }\DIFaddend into the existing \texttt{\$HDXER\_PATH/tutorials/BPTI/} folder. 
However, for users short on time or storage space, we also provide a reference set of \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{computed }\DIFaddend HDX-MS data, which can be used to follow the visualization and analysis steps of the tutorial. 
...
\noindent
\textit{\DIFdelbegin \DIFdel{Predicting }\DIFdelend \DIFaddbegin \DIFadd{Computing }\DIFaddend HDX data \DIFdelbegin \DIFdel{from }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend MD \DIFdelbegin \DIFdel{simulations}\DIFdelend \DIFaddbegin \DIFadd{simulation snapshots}\DIFaddend } -- In this tutorial we introduce the \texttt{calc\_hdx.py} Python script (see Figure \ref{fig:workflowfig}), a wrapper script that allows users to \DIFdelbegin \DIFdel{predict }\DIFdelend \DIFaddbegin \DIFadd{compute }\DIFaddend HDX-MS data for \DIFaddbegin \DIFadd{each frame in }\DIFaddend an ensemble of structures. 
This script is run using a Unix command line interface and allows the input filenames, method \DIFaddbegin \DIFadd{type}\DIFaddend , analysis options, and output filenames to be specified using \textit{via} command-line flags.

In the first step of the tutorial we set up and describe the \texttt{calc\_hdx.py} command to \DIFdelbegin \DIFdel{predict }\DIFdelend \DIFaddbegin \DIFadd{calculate }\DIFaddend HDX-MS data from the BPTI MD trajectories.
We encourage users to review the help text provided by \texttt{calc\_hdx.py} (accessible by running \texttt{calc\_hdx.py -h} on the command line), which explains how to \DIFdelbegin \DIFdel{provide input data, specify method options, and name outputs, using }\DIFdelend \DIFaddbegin \DIFadd{use }\DIFaddend the command line flags.

Next, we run \texttt{calc\_hdx.py} to \DIFdelbegin \DIFdel{predict }\DIFdelend \DIFaddbegin \DIFadd{compute }\DIFaddend HDX-MS data from the BPTI MD trajectories, using a default set of \DIFdelbegin \DIFdel{prediction }\DIFdelend options.
Specifically, we utilize the \textit{BestVendruscolo} method flag, a \DIFaddbegin \DIFadd{predictive }\DIFaddend model named after its developers \cite{BestVendruscolo2006}, to \DIFdelbegin \DIFdel{predict }\DIFdelend \DIFaddbegin \DIFadd{compute }\DIFaddend protection factors.
As \DIFdelbegin \DIFdel{we first showed }\DIFdelend \DIFaddbegin \DIFadd{shown }\DIFaddend in Equation \ref{pf_model}, this model uses the number of H bonds and heavy atom contacts formed by each amide NH group to calculate the protection factor for each backbone amide.
By default, HDXer evaluates H bonds and contacts in an identical way to the originally-parameterized model: an H bond is counted if there is a protein oxygen atom found within 2.4 Å of the amide H atom, \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{while }\DIFaddend heavy-atom contacts are computed as the total number of nearby protein non-hydrogen atoms \DIFdelbegin \DIFdel{, }\DIFdelend within a radius of 6.5 Å of the amide N atom.
...
The final series of \texttt{calc\_hdx.py} options that users are invited to select during the tutorial \DIFdelbegin \DIFdel{, }\DIFdelend relates to the intrinsic exchange rates for each backbone amide.
Intrinsic exchange rate constants, $k_{int}$ \DIFdelbegin \DIFdel{, }\DIFdelend will be computed by HDXer automatically, based upon the amino acid sequence of the protein provided to \texttt{calc\_hdx.py} and the temperature and pD conditions at which the experimental data was measured. 
...
Finally, to complete the calculation of exchange rates as per \DIFdelbegin \DIFdel{equation }\DIFdelend \DIFaddbegin \DIFadd{Equation }\DIFaddend \ref{Df_eqn}, the user should provide a list of labeling times that reflect the protocol used to generate the experimental data, or else default values will be used.

\noindent
\textit{Plotting \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{computed }\DIFaddend HDX data} -- The agreement between the \DIFdelbegin \DIFdel{newly-predicted HDX data }\DIFdelend \DIFaddbegin \DIFadd{HDX data inferred from the structural ensemble }\DIFaddend and the experimental data can be assessed using plots of the deuterated fraction for each peptide fragment.
The tutorial therefore guides users through the creation of such plots, as well as the calculation of some simple descriptive statistics to characterize the agreement between \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{computed }\DIFaddend and experimental data. 
The output for BPTI illustrates good agreement for most peptides in the dataset, with an overall $R^2$ for the predictions of 0.59 \DIFdelbegin \DIFdel{, }\DIFdelend and a RMSE between the \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{computed }\DIFaddend and experimental fractional deuteration values of 0.30. 

\subsubsection{Conclusion}

During Tutorial 2 we \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{calculated }\DIFaddend HDX data from an ensemble of structures of BPTI and compared the results to experimental HDX data. 
In the case of BPTI, the \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{computed }\DIFaddend and experimental HDX data correlate reasonably well ($R^2$ = 0.59), which is not uncommon, but offers clear scope for improvement.
In cases when the correlation is poor, even for only a few individual residues, it can be challenging to distinguish between the multiple possible sources of mismatch\DIFdelbegin \DIFdel{, which }\DIFdelend \DIFaddbegin \DIFadd{. 
These sources }\DIFaddend include errors in the experimental data or the HDX prediction model as well as limitations of sampling in MD simulations. 
...
By allowing for reweighting of the structural ensemble, HDXer can \DIFdelbegin \DIFdel{robustly }\DIFdelend \DIFaddbegin \DIFadd{rigorously }\DIFaddend account for all these sources of error. 
...
As mentioned above, the process of reweighting applies a minimal bias to the candidate ensemble to identify a structural ensemble that conforms to the experimental HDX data within a given level of uncertainty (\DIFdelbegin \DIFdel{equation }\DIFdelend \DIFaddbegin \DIFadd{Equation }\DIFaddend \ref{KL_eqn}).
...
In practice, to avoid overfitting, reweighting should be carried out in multiple iterations in which $\gamma$ values are varied\DIFdelbegin \DIFdel{, and the }\DIFdelend \DIFaddbegin \DIFadd{. 
The }\DIFaddend accuracy of the reweighted data (e.g. the RMSE between the \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{computed }\DIFaddend and target HDX) and the bias applied (in terms of $W_\textnormal{app}$) should \DIFaddbegin \DIFadd{then }\DIFaddend be evaluated for each iteration.
...
Files containing the target experimental HDX-MS data, the intrinsic rates for each protein residue present in the candidate structures, and the residue-level contacts and H bonds used for the \DIFdelbegin \DIFdel{HDX predictions}\DIFdelend \DIFaddbegin \DIFadd{computed HDX values}\DIFaddend , are all required for HDX ensemble reweighting calculations. 
Furthermore, the deuterium labeling timepoints associated with the target experimental data are also necessary\DIFdelbegin \DIFdel{, although they }\DIFdelend \DIFaddbegin \DIFadd{; these }\DIFaddend are provided as keyword arguments to HDXer in a Python script, rather than in a separate file. 
Thus, the user should have completed the previous sections of the HDXer tutorials to ensure \DIFaddbegin \DIFadd{that }\DIFaddend they are aware of the contents of \DIFdelbegin \DIFdel{these }\DIFdelend \DIFaddbegin \DIFadd{the required }\DIFaddend files and how they will be \DIFdelbegin \DIFdel{utilized within the }\DIFdelend \DIFaddbegin \DIFadd{used during }\DIFaddend HDX ensemble reweighting\DIFdelbegin \DIFdel{process}\DIFdelend . 
...
As such, users can straightforwardly customize and carry out reweighting by writing only a few lines of Python code \DIFaddbegin \DIFadd{that make use of this module}\DIFaddend .
Notebook \texttt{03\_reweighting.ipynb} provides an example of commands to \DIFdelbegin \DIFdel{, first , }\DIFdelend \DIFaddbegin \DIFadd{first }\DIFaddend set up a reweighting analysis as a \texttt{HDXer.reweighting.MaxEnt} object \DIFdelbegin \DIFdel{, }\DIFdelend and subsequently to run reweighting interactively across a range of $\gamma$ values \DIFdelbegin \DIFdel{, }\DIFdelend from $\gamma = 1\times10^{-3}$ to $\gamma = 9\times10^{-3}$.

The value of $\gamma$ ultimately determines the level of uncertainty allowed in the fit of the \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{computed }\DIFaddend HDX data to the target experimental data. 
...
Consequently, the discrepancy, or error, between the final \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{computed }\DIFaddend HDX data from the reweighted ensemble and the target experimental HDX data will be reduced.

After performing ensemble reweighting, HDXer produces output files containing the following data: the individual weights (i.e., relative probabilities) of each structure in the ensemble; the final \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{computed }\DIFaddend deuterated fractions for that reweighted ensemble; the mean squared error between the \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{computed }\DIFaddend and target experimental HDX data; and finally, the 'apparent work' that was applied as a bias to the ensemble as a whole.
...
In notebook \texttt{04\_decision\_plot.ipynb} we therefore create a decision plot of $W_{app}$ \DIFdelbegin \textit{\DIFdel{versus}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{against }\DIFaddend the mean squared error between the \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{computed }\DIFaddend and experimental HDX data (Figure \ref{fig:decision_plot}).
The most suitable $\gamma$ for the subsequent analyses, i.e., that producing the closest agreement to experiment without inducing overfitting, can be identified by looking for a sharp elbow, \DIFaddbegin \DIFadd{or }\DIFaddend 'L', shape in the decision plot.
...
\begin{figure}
    ...
    \caption{Decision plot for the BPTI MD simulation trajectories. An L-shaped curve reveals a sharp increase in work \DIFdelbeginFL \DIFdelFL{versus }\DIFdelendFL \DIFaddbeginFL \textit{\DIFaddFL{versus}} \DIFaddendFL the mean squared error, suggesting potential overfitting at values of $\gamma$ \DIFdelbeginFL \DIFdelFL{leading to higher }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for which }\DIFaddendFL work values \DIFaddbeginFL \DIFaddFL{are higher }\DIFaddendFL than that at the inflection point. Here, a $\gamma$ value of 2.0 is selected based on the shape of the curve and \DIFaddbeginFL \DIFaddFL{on }\DIFaddendFL a cutoff of 4.0 kJ mol$^{-1}$ in the apparent work value (dashed line). The weights obtained at this value of $\gamma$ will be used for further structural interpretation.}
    \label{fig:decision_plot}
\end{figure}

Here, we keep the work value below 4 kJ mol$^{-1}$ (roughly 1 kcal mol$^{-1}$) \DIFdelbegin \DIFdel{, }\DIFdelend and choose a $\gamma$ value of $2\times10^0$, based on Figure \ref{fig:decision_plot}.
To visualize the effects of the bias applied at this $\gamma$ value, we plot an overlay of the \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{computed }\DIFaddend deuteration values before and after reweighting, which demonstrates how the accuracy of the \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{computed }\DIFaddend data (compared to the experimental HDX) improves as a result of reweighting.
An example is shown for the 1-minute labeling time in \DIFdelbegin \DIFdel{Fig. }\DIFdelend \DIFaddbegin \DIFadd{Figure }\DIFaddend \ref{fig:lineplot}.
Finally, we recognize that the choice of $\gamma$ at which to evaluate reweighting is a nuanced decision \DIFdelbegin \DIFdel{, }\DIFdelend and may not always be as straightforward a choice as for the reweighting of BPTI presented in the tutorial.
...
\noindent
\textit{Plotting a heatmap} -- After performing reweighting, at the end of notebook \texttt{04\_decision\_plot.ipynb}, users create line plots comparing the \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{computed }\DIFaddend values with the target experimental data at each labeling time (see \DIFdelbegin \DIFdel{Fig. }\DIFdelend \DIFaddbegin \DIFadd{Figure }\DIFaddend \ref{fig:lineplot}), providing a simple visualization of how reweighting has improved the agreement of the \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{computed }\DIFaddend HDX-MS data with the target data.
However, such visualizations do not readily allow comparison of data from multiple time-points and multiple conditions simultaneously \cite{Masson2019}\DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{. 
Such plots also }\DIFaddend neglect the structural context of the HDX-MS \DIFdelbegin \DIFdel{predictions, }\DIFdelend \DIFaddbegin \DIFadd{calculations }\DIFaddend and thereby the impact of reweighting on the structural ensemble itself.
...
\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD <     \includegraphics[width=0.98\linewidth]{Fig5_BPTI_lineplot_1min_v2.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \begin{figure*}[ht]
    \centering
    \includegraphics[width=0.9\linewidth]{Fig5_BPTI_lineplot_1min_v4.pdf}
    \DIFaddendFL \caption{Target fractional deuteration values for selected residues in BPTI after 1 minute labeling (orange) overlaid with \DIFdelbeginFL \DIFdelFL{predicted }\DIFdelendFL values obtained \DIFaddbeginFL \DIFaddFL{for a structural ensemble }\DIFaddendFL before reweighting (dark blue) and after reweighting (light blue). Although H-D exchange is predicted accurately for many residues, some discrepancies between prediction and experiment remain, even following reweighting.}
    \label{fig:lineplot}
...
In this case, the columns correspond to residues in BPTI for which HDX-MS data was \DIFdelbegin \DIFdel{predicted}\DIFdelend \DIFaddbegin \DIFadd{computed}\DIFaddend , while the rows represent the time points at which exchange was \DIFdelbegin \DIFdel{predicted}\DIFdelend \DIFaddbegin \DIFadd{computed}\DIFaddend .
Each pixel is then colored by the \DIFaddbegin \DIFadd{the difference between the calculated deuteration before and after reweighting, }\DIFaddend $\Delta{D_{i,t}} = D_{i,t,\textnormal{final}} - D_{i,t,\textnormal{initial}}$, \DIFdelbegin \DIFdel{meaning }\DIFdelend \DIFaddbegin \DIFadd{such }\DIFaddend that regions colored red exhibit higher levels of deuteration as a result of reweighting, while regions colored blue are predicted to have lower levels of deuteration.
...
At this point, users may wish to probe the structural effects of reweighting in more depth \DIFdelbegin \DIFdel{, }\DIFdelend with additional, independent, analyses of the reweighted structures present in the final ensemble. 
For example, in this case, the impact of reweighting on the final \DIFdelbegin \DIFdel{predicted deuterated fractions , represented by the difference between the predicted deuteration before and after reweighting (}\DIFdelend \DIFaddbegin \DIFadd{calculated deuterated fractions (represented by }\DIFaddend $\Delta$D) \DIFdelbegin \DIFdel{, }\DIFdelend is relatively small across the different labeling times, but is not uniformly distributed across the entire protein. 
In particular, reweighting \DIFdelbegin \DIFdel{does }\DIFdelend \DIFaddbegin \DIFadd{did }\DIFaddend not seem to affect the \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{calculated }\DIFaddend deuteration for residues 19 to 35, some of which showed large discrepancies \DIFaddbegin \DIFadd{compared }\DIFaddend to the experimental target data initially (\DIFdelbegin \DIFdel{Fig. }\DIFdelend \DIFaddbegin \DIFadd{Figure }\DIFaddend \ref{fig:lineplot}). 
The residual error between the \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{calculated }\DIFaddend and experimental data in these regions may arise from a variety of sources\DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{; }\DIFaddend we will discuss possible approaches to delineate the cause of these discrepancies below, in \DIFdelbegin \DIFdel{section }\DIFdelend \DIFaddbegin \DIFadd{Section }\DIFaddend \ref{robustness_sect}.
Nevertheless, for other regions of BPTI, the final \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{calculated }\DIFaddend HDX-MS values are in good agreement with the target data, illustrated by closely matching deuteration levels in \DIFdelbegin \DIFdel{Fig. \ref{fig:lineplot}, which }\DIFdelend \DIFaddbegin \DIFadd{Figure \ref{fig:lineplot}.
This observation }\DIFaddend suggests that the reweighting approach has led to a reasonable description of the structure and dynamics of these regions in the final reweighted ensemble.

\DIFdelbegin %DIFDELCMD < \begin{figure}
%DIFDELCMD <     \includegraphics[width=0.98\linewidth]{Fig6_heatmap+structures.png}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \begin{figure*}[t]
    \centering
    \includegraphics[width=0.9\linewidth]{Fig6_heatmap+structures_2col_v2.png}
    \DIFaddendFL \caption{Effect of reweighting on the \DIFdelbeginFL \DIFdelFL{predicted }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{calculated }\DIFaddendFL deuteration levels in the BPTI structural ensemble. (A) Difference in \DIFdelbeginFL \DIFdelFL{predicted }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{calculated }\DIFaddendFL HDX-MS for each residue at different labeling times shown as a heat map. Red represents an increase in deuteration and blue represents a decrease in deuteration after reweighting. (B) For each labeling timepoint, the difference in HDX is mapped onto an example BPTI crystal structure (protein databank entry 5PTI).}
    \label{fig:heatmap}
\DIFdelbeginFL %DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \end{figure*}
...
In Tutorial 3, we performed HDX ensemble reweighting for BPTI with simple Python scripts \DIFdelbegin \DIFdel{, }\DIFdelend and tracked the accuracy of the \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{calculated }\DIFaddend HDX-MS data and the total bias applied to the candidate ensemble across a range of $\gamma$ values.
We discussed how users can choose a suitable $\gamma$ value at which to analyze the final reweighted results \DIFdelbegin \DIFdel{, }\DIFdelend and provided examples of simple analyses \DIFdelbegin \DIFdel{and visualizations }\DIFdelend that users can apply to interpret their data.
Although the examples provided in Tutorial 3 are by no means exhaustive, at this point we expect users will be familiar with the basics of performing HDX ensemble reweighting and analyzing the outcomes \DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{and be }\DIFaddend ready to apply the approach to their own systems and target datasets.
...
A crucial final step for HDXer analyses, to which we have alluded in many sections above, is to ensure that the outputs from reweighting analyses are robust \DIFdelbegin \DIFdel{, }\DIFdelend and that conclusions can be drawn from the reweighted structural ensemble with confidence.
...
An optimal approach to probe the accuracy of HDXer results would be to cross-validate the reweighted structural ensemble against other structural or functional data \DIFdelbegin \DIFdel{, for example from complementary solution-state structural biology techniques, }\DIFdelend and to assess the consistency of the interpretations thereof.
\DIFdelbegin \DIFdel{However, in the }\DIFdelend \DIFaddbegin \DIFadd{For example, the final reweighted structural ensemble could be used to make predictions of complementary solution-state structural data such as SAXS, SANS, or NMR, or to predict specific structural features such as distance distributions observed in DEER, FRET, or crosslinking-MS experiments.
In making these predictions, users should remember that HDXer applied with the phenomenological predictive model here \mbox{%DIFAUXCMD
\cite{BestVendruscolo2006} }\hspace{0pt}%DIFAUXCMD
characterizes predominantly the amide C state structures and dynamics (Section \ref{predictivemodel_sect}).
...
Nevertheless, as precise exchange mechanisms may \DIFdelbegin \DIFdel{vary }\textit{\DIFdel{within}} %DIFAUXCMD
...
If the predictive model is not representative of a particular experimental exchange mechanism in the protein, errors in HDX-MS \DIFdelbegin \DIFdel{predictions }\DIFdelend \DIFaddbegin \DIFadd{calculations }\DIFaddend will likely correlate with specific protein regions, rather than being uniformly distributed across the whole range of target data.
...
This would \DIFdelbegin \DIFdel{include }\DIFdelend \DIFaddbegin \DIFadd{involve }\DIFaddend all datapoints that correspond to a particular location according to the sequence or structure \DIFdelbegin \DIFdel{, }\DIFdelend and may include multiple, overlapping, peptide segments.
For each repeated reweighting analysis, users can subsequently optimize the predictive model $\beta_\textnormal{C}$ and $\beta_\textnormal{H}$ parameters for the remaining protein regions, either by gradient optimization, Monte Carlo optimization, or Monte Carlo sampling\DIFdelbegin \DIFdel{, which }\DIFdelend \DIFaddbegin \DIFadd{; the specific optimization routine }\DIFaddend can be selected using corresponding Python arguments to HDXer as exemplified in the tutorial notebooks.
...
By contrast, if the optimal parameters for individual regions vary beyond the range that the user would expect, according to the parameter exploration of \DIFdelbegin \DIFdel{section }\DIFdelend \DIFaddbegin \DIFadd{Section }\DIFaddend \ref{choosingbetaparams_sect} (e.g. Figure \ref{fig:contourplot}), \DIFdelbegin \DIFdel{this might suggest }\DIFdelend \DIFaddbegin \DIFadd{it is possible }\DIFaddend that a single predictive model \DIFdelbegin \DIFdel{cannot }\DIFdelend \DIFaddbegin \DIFadd{does not }\DIFaddend estimate $P_i$ for \DIFdelbegin \DIFdel{some }\DIFdelend \DIFaddbegin \DIFadd{all those }\DIFaddend protein regions with equal accuracy.
...
\subsection{Uncertainty resulting from \DIFaddbegin \DIFadd{the }\DIFaddend candidate ensemble}\label{ensemble_uncertainty_sect}
The final source of error in HDXer analyses is perhaps the most straightforward for users to identify and characterize.
As \DIFdelbegin \DIFdel{we have }\DIFdelend referenced throughout the tutorials, an ideal candidate ensemble for reweighting should cover all 'experimentally-relevant' protein conformational states but contain very few \DIFdelbegin \DIFdel{conformations not present in the experiment.
Extraneous , non-experimental, }\DIFdelend \DIFaddbegin \DIFadd{'irrelevant' conformations.
Extraneous }\DIFaddend protein conformations pose a challenge because they must be downweighted in order to amplify the 'signal' of structural motifs that conform well to the experimental HDX-MS data.
This 'structural noise' can arise either from insufficient sampling in the generation of the candidate ensemble, or from systematic sampling of conformations that are not \DIFdelbegin \DIFdel{present in the experimental ensemble }\DIFdelend \DIFaddbegin \DIFadd{relevant to fitting of the target data }\DIFaddend (for example, if apo conformations are included in an ensemble being reweighted to fit HDX-MS data of a holo protein-ligand complex).
...
Random uncertainty in the HDXer \DIFdelbegin \DIFdel{predictions }\DIFdelend \DIFaddbegin \DIFadd{calculations }\DIFaddend can be characterized by repeating identical HDXer analyses with smaller random subsamples (\textit{ca.} 10-33\% of the full population) of the candidate ensemble.
...
To test for systematic errors in the candidate structures, users should begin by performing a thorough structural analysis\DIFaddbegin \DIFadd{, for example using dimensionality reduction or clustering techniques, }\DIFaddend in order to classify and quantify each conformational state present in their initial ensemble.
Then, following a similar \DIFaddbegin \DIFadd{leave-one-out }\DIFaddend approach to that used above to identify systematic errors in the target data, users can perform repeated HDXer analyses using subsections of the candidate ensemble, systematically excluding each classified conformational state from the initial ensemble. 
\DIFdelbegin \DIFdel{The contribution of }\DIFdelend \DIFaddbegin \DIFadd{Reweighting with }\DIFaddend the remaining conformational states can then be quantified with respect to the final $W_{\textnormal{app}}$ value, the RMSE to the target data, and to the resultant reweighted populations, whether up- or down-weighted.
\DIFdelbegin \DIFdel{If excluding a particular conformation from the candidate ensemble results in reweighting with either (a) improved (smaller) RMSE to target with an equivalent applied }\DIFdelend \DIFaddbegin \DIFadd{Each trial candidate ensemble may lead to either improved (i.e. smaller }\DIFaddend $W_{\textnormal{app}}$, \DIFdelbegin \DIFdel{or (b) improved (smaller ) }\DIFdelend \DIFaddbegin \DIFadd{smaller RMSE) or worsened (i.e. larger }\DIFaddend $W_{\textnormal{app}}$\DIFdelbegin \DIFdel{required to achieve an equivalent RMSE}\DIFdelend \DIFaddbegin \DIFadd{, larger RMSE) fit }\DIFaddend to the target data\DIFdelbegin \DIFdel{, then the excluded conformation is unlikely to be present in the experimental ensembleand can be safely excluded from the interpretation of the target HDX-MS data.
...
Finally}\DIFaddend , if large RMSE to the target data remains, regardless of the selected conformational ensemble and applied $W_{app}$, this might suggest that neither any structural state sampled by the MD simulation, nor any population mixture of sampled states, represents the experimental ensemble well.
...
%DIF < %%%%%%%%%%%%%%%
%DIF <  This section must describe the actual contributions of
%DIF <  author. Since this is an electronic-only journal, there is
%DIF <  no length limit when you describe the authors' contributions,
%DIF <  so we recommend describing what they actually did rather than
%DIF <  simply categorizing them in a small number of
%DIF <  predefined roles as might be done in other journals.
%DIF < 
%DIF <  See the policies ``Policies on Authorship'' section of https://livecoms.github.io
%DIF <  for more information on deciding on authorship and author order.
%%%%%%%%%%%%%%%%
