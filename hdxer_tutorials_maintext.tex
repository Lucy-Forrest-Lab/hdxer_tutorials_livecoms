...
\orcid{Suhwan Paul Lee}{0000-0003-2658-4641}
...
In particular, experiments that couple HDX to mass spectrometry measurements of the deuterium uptake (HDX-MS) have become the \textit{de facto} standard for applications of HDX to the dynamics of proteins in solution, allowing determination of exchange data for large protein systems or complexes and with low sample amounts \cite{Oganesyan2018, Engen2020}.
...
In terms of background knowledge, we begin here with a general introduction to the theoretical concepts of both HDX and ensemble reweighting, while suggesting sources of more detailed information for the interested user.
...
Since the environment and dynamics of each backbone amide in a protein chain are unique, the underlying structural and chemical mechanisms of exchange within a protein may vary \cite{Englander1983, Skinner2012mechanisms, Skinner2012models}.
As such, there are no rigorous, generalized structural determinants of exchange.
However, a number of predictive models have been developed to correlate protein structural and dynamical metrics with protection factors \cite{VendruscoloPaci2003, BestVendruscolo2006, Kieseritzky2006, Ma2011, Liu2012, Petruk2013, Park2015, Persson2015, Markwick2019}.
The protection factors estimated by these models may then be used to calculate exchange rates, and ultimately deuterium uptake, for peptides observed to follow the EX2 exchange kinetic regime in HDX-MS experiments.
...
\subsection{Additional tutorial prerequisites}
\subsubsection{Background knowledge}
In addition to a solid foundation in the theory underpinning ensemble reweighting and computational HDX predictions, users should have a good understanding of the two data sources in their HDXer analyses, namely structural ensembles generated \textit{via} molecular modeling and simulation, and the experimental measurement of protein HDX-MS data.
...
HDXer analyses assume that the target exchange data has been normalized for back exchange and for the total deuterium ratio in the solution (e.g., by normalizing uptake to a fully-deuterated control sample), and that the data represent residues and peptides undergoing an EX2-type exchange mechanism.
...
For BPTI, we provide in-house MD simulation trajectories in a repository on Zenodo (\url{doi:10.5281/zenodo.4640761}). 
...
The previous sections provide a number of practical methods to assess and rectify any ambiguities in the computational reweighting applied by HDXer.
...
For example, if the HDX-MS experiments are known to probe a highly diverse conformational ensemble, with many states of varied population, it may be difficult to validate that HDXer reweighted ensembles comprise all the relevant conformational states without lengthy testing.
...
Users should carefully choose protein/ligand concentrations, equilibration times, and dilution ratios to ensure that the system studied is a) at equilibrium, and b) as much as possible monodisperse, during the experiment.
Experimental solutions that contain a mixture of conformational states, or states that interconvert over experimental timescales, may give rise to HDX-MS signals that are particularly challenging to interpret.
...
Data should be inspected and carefully filtered to remove as much uncertainty as possible from the measured deuterium uptake values.
Selecting peptides based on intensity (i.e. signal/noise ratio), and monitoring for artefacts (e.g. signs of aggregation) in mass envelopes, can minimize uneven redundancy throughout the protein and maximize the representation of peptides with state-to-state differences.
In addition, peptide MS data should be monitored for the presence of EX1 or mixed EX1/EX2 kinetic regimes.
Predictive protection factor models are only suited to interpret EX2-type behavior, and thus peptides that exhibit alternate regimes should be excluded from analysis.
Dependent on the experimental design, peptides can manifest different exchange regimes across the timescale of an HDX-MS experiment.
In this case, users should consider removing all data from labeling times at which EX1 behavior is observed, in order to minimize the uncertainty present in the remaining (shorter) experimental labeling time data.

The suggestions above will aid the selection of reliable HDX-MS data as a target for reweighting analyses, but they are unable to guarantee that each datapoint is uncertainty-free.
...
Instead, well-characterised, high-quality experimental data will simplify the quality control steps of HDXer analyses, and enhance the accuracy and reliability of the structural interpretations of HDX-MS data.
Accordingly, we recommend that both parts of the analysis, experimental and computational, are considered and designed together.
This will create a single, integrative, HDX-MS and HDXer workflow, and, in our experience, is the most likely approach for users to successfully and unambiguously answer biological questions of interest.
